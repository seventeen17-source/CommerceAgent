...
- **T022 review hardening（待本地复验）**：在原 258 passed / 6 skipped 基线上继续补 3 个恢复边界：① `AfterSalesStatus.refunds/returns` 改为必填，缺字段不得伪装成空列表；② unknown-write recovery 用 `idempotencyKey` 过滤权威状态，禁止把同订单上别的退款误认成当前写入；③ `WriteIntent` 增加稳定 request fingerprint，并把 `intent + PENDING` 定义为一次持久化调用，恢复 payload 漂移时在发请求前 fail closed。**这批补丁尚未记录新的本地门禁结果，进入 T023 前需重跑 Python 四条门禁。**
