 ...
 *
 * <ul>
 *   <li>该表**没有** `version` 列，V1 因此把明细业务字段视为创建后不可变，本实体也不提供 quantity/unit_price 等 setter。
 *       注意：{@code Order.@Version} 不会自动保护对子表 `order_items` 的直接 UPDATE；如果未来允许修改明细，必须显式补充
 *       明细自己的乐观锁，或设计能够可靠推进订单版本号的并发控制方案，不能假设聚合根版本会自动兜底。
