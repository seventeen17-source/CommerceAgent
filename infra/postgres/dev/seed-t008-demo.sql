...
-- All demo identifiers use the demo- prefix. The script is idempotent.
-- Approximate size after one run:
...
SELECT
    ...
    '2026-09-03 09:00:00+08'::timestamptz + ((i - 3) * interval '1 day')
...
VALUES
    ...
    (
        ...
        '2026-09-08 11:00:00+08', '2026-09-09 08:30:00+08',
        '2026-09-12 14:20:00+08', NULL, 0
    ),
    (
...
SELECT
    ...
    'CNY',
    '2026-08-20 10:00:00+08'::timestamptz + (i * interval '1 day'),
    CASE
        WHEN i % 5 IN (2, 3, 0)
            THEN '2026-08-20 10:00:00+08'::timestamptz + ((i + 1) * interval '1 day')
        ELSE NULL
    END,
    CASE
        WHEN i % 5 IN (3, 0)
            THEN '2026-08-20 10:00:00+08'::timestamptz + ((i + 3) * interval '1 day')
        ELSE NULL
    END,
    NULL,
    0
...
SELECT
    ...
    CASE
        WHEN i % 5 = 2
            THEN '2026-08-20 10:00:00+08'::timestamptz + ((i + 2) * interval '1 day')
        ELSE '2026-08-20 10:00:00+08'::timestamptz + ((i + 3) * interval '1 day')
    END,
    CASE
        WHEN i % 5 IN (3, 0)
            THEN '2026-08-20 10:00:00+08'::timestamptz + ((i + 3) * interval '1 day')
...
INSERT INTO commerce.logistics_events (shipment_id, event_type, description, occurred_at)
SELECT 'demo-shipment-delivered-001', e.event_type, e.description, e.occurred_at
FROM (VALUES
    ('PICKED_UP', 'Package picked up by carrier', '2026-09-09 09:00:00+08'::timestamptz),
    ('OUT_FOR_DELIVERY', 'Courier is delivering the package', '2026-09-12 09:10:00+08'::timestamptz),
    ('DELIVERED', 'Package signed by recipient', '2026-09-12 14:20:00+08'::timestamptz)
) AS e(event_type, description, occurred_at)
WHERE NOT EXISTS (
    SELECT 1 FROM commerce.logistics_events le
    WHERE le.shipment_id = 'demo-shipment-delivered-001'
      AND le.event_type = e.event_type
      AND le.occurred_at = e.occurred_at
);
...
SELECT
    ...
    '2026-08-20 10:00:00+08'::timestamptz
        + ((i + e.day_offset) * interval '1 day')
        + (e.hour_offset * interval '1 hour')
FROM generate_series(1, 15) AS g(i)
CROSS JOIN (VALUES
...
VALUES
    ('DEMO_STALLED_72H_REFUND', 1, NULL, 'SHIPPED', 72, NULL, 500.00, 300.00,
     'REFUND_ONLY', TRUE, '2026-01-01 00:00:00+08', NULL),
    ('DEMO_DELIVERED_7D_RETURN', 1, NULL, 'DELIVERED', NULL, 7, 500.00, 300.00,
     'RETURN_REFUND', TRUE, '2026-01-01 00:00:00+08', NULL),
    ('DEMO_HIGH_VALUE_MANUAL', 1, 'electronics', NULL, NULL, NULL, 2000.00, 1000.00,
     'MANUAL_REVIEW', TRUE, '2026-01-01 00:00:00+08', NULL),
    ('DEMO_ACCESSORY_14D_RETURN', 1, 'accessories', 'DELIVERED', NULL, 14, 300.00, 250.00,
     'RETURN_REFUND', TRUE, '2026-01-01 00:00:00+08', NULL),
    ('DEMO_HOME_30D_RETURN', 1, 'home', 'DELIVERED', NULL, 30, 800.00, 600.00,
     'RETURN', TRUE, '2026-01-01 00:00:00+08', NULL),
    ('DEMO_CLOSED_ORDER_DENY', 1, NULL, 'CLOSED', NULL, NULL, 0.00, 0.00,
     'DENY', TRUE, '2026-01-01 00:00:00+08', NULL),
    ('DEMO_APPAREL_REVIEW', 1, 'apparel', NULL, NULL, 14, 1000.00, 500.00,
     'MANUAL_REVIEW', TRUE, '2026-01-01 00:00:00+08', NULL)
...
VALUES (
    ...
    'demo-customer-001', 'COMPLETED', 'CHECK_LOGISTICS', 'demo-order-stalled-001',
    'verify_evidence', NULL, 2, 0,
    '{"demo": true, "note": "synthetic T008 visualization only"}'::jsonb,
    'EXPLAIN_STALLED_LOGISTICS', NULL, 'demo-model', 0.000, 'demo-v1',
    120, 56, '2026-09-17 20:00:00+08', '2026-09-17 20:00:02+08'
)
...
SELECT
    ...
    100 + i * 17,
    40 + i * 9,
    '2026-09-17 20:10:00+08'::timestamptz + (i * interval '5 minutes'),
    CASE
        WHEN i IN (1, 4, 5, 6)
            THEN '2026-09-17 20:10:00+08'::timestamptz
                + (i * interval '5 minutes') + interval '3 seconds'
        ELSE NULL
    END
...
SELECT
    ...
    '2026-09-17 20:10:00+08'::timestamptz
        + (i * interval '5 minutes') + (s.step_index * interval '1 second')
FROM generate_series(1, 7) AS g(i)
CROSS JOIN (VALUES
    (1, 'get_order', 'LOW'),
    (2, 'get_logistics', 'LOW'),
...
SELECT
    ...
    '2026-09-17 21:00:00+08'::timestamptz + (i * interval '1 minute')
FROM generate_series(1, 15) AS g(i)
WHERE NOT EXISTS (
    SELECT 1 FROM commerce.audit_logs al
...
WITH demo_counts AS (
    ...
    FROM agent.tool_executions WHERE trace_id LIKE 'demo-trace-%'
)
