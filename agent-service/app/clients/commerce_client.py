...
class CommerceClient:
    ...
    async def _send(
        ...
        *,
        ...
        if not response.is_success:
            raise self._failure(response, method=method, path=path, request_is_safe=request_is_safe)
        try:
    ...
    def _failure(
        self, response: httpx.Response, *, method: str, path: str, request_is_safe: bool
    ) -> CommerceError:
