...
当某个 Txxx 需要通过 `http://localhost:5173/` 做人工或端到端验证时，页面承担的是“到目前为止系统已经具备哪些可执行能力”的教学职责。

- T016 Flow Playground 是稳定基线；后续任务在同一条可视化调用链上增加测试按钮、checkpoint/trace 查看器或 live mode，不重新开独立任务页面；
- Playground 的三个固定教学区域是：上方“故障注入”选择测试场景，左侧“调用链”累计主流程步骤/接口，右侧 `STEP DETAIL` 展示所选步骤的输入、行为、输出和失败状态，并直接承载该步骤的真实请求控件与原始响应；
- 新的主链能力（如创建 Run、读取 Checkpoint、选择 Tool、执行/验证退款、HITL）按执行顺序追加到左侧调用链；辅助接口和同一步内部调用进入对应步骤右侧详情，避免把接口数量误当成教学步骤数量，也不得在页面底部另开 Txxx 验证区；
- 内部实现可在 `web/src/features/validation/` 按任务拆分组件，但 `App.tsx` 对用户只呈现一个连续的 CommerceAgent 调试器；
- 新能力不得覆盖旧能力。完成 T019 后，用户仍应能切回 T016 mock 故障模式并执行 T018 Run 查询；
- 页面验证结果必须能映射回该任务的层、输入、输出、数据库或 Trace 证据，避免页面沦为“只看到成功提示”的演示壳。
