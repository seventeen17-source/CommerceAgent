...
- [ ] T017 实现持久化 Agent Run/Checkpoint/Structured Tool Trace：`agent-service/app/trace/`；支持 `RUNNING`、`WAITING_USER`、`WAITING_APPROVAL`、`COMPLETED`、`ESCALATED`、`FAILED`、`SAFE_STOP`；Tool Trace 必须持久化 `run_id + step_index + trace_id`（以及 error/retry/write-verification 摘要），确保 failed run 可定位到具体跨服务 Java 请求；raw token 不得进入 trace；定义 terminal run 的 retention/cleanup policy，运行中/等待中的 checkpoint 可恢复，终态详细 checkpoint/tool trace 不得无限期无界增长。
