...
- **T016 post-review hardening 已最终本地复验通过（2026-09-21）**：错误响应与成功响应共用 `_correlate()` 产出的已校验 correlation id；JSON error envelope 的 `traceId` 不再覆盖最终 `CommerceApiError.trace_id`。补充 `test_client_auth.py`，直接钉死 credential ≠ identity、CR/LF/whitespace 拒绝、SecretStr 脱敏、额外 `user_id/role` 拒绝。用户已重新执行 `ruff check .`、`ruff format --check .`、`mypy app`、`pytest -q`，**四条全部成功**；本次未记录新的 pytest 精确总数，因此不伪造数字。T016 至此封账，当前任务保持 T017。
